...
<FormPage
  ...
  {#snippet content()}
    <div class="flex flex-col w-full">
      {#await Promise.all([data.container, data.alternative, data.localImage])}
        <!-- loading indicator -->
        <div class="h-0.5">
          <ProgressBar class="w-full h-0.5" width="w-full" height="h-0.5" />
        </div>
      {:then [container, alternative, localImage]}
        <CloneForm container={container} alternative={alternative} localImage={localImage} close={close} />
      {:catch error}
